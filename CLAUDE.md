...
- `vocab-enrichment.js` - 語彙拡充データ。2つのconstを持つ: ①`VOCAB_ENRICHMENT`（第2例文・語源/覚え方・コロケーション・類義語`syn`・ニュアンス/使用場面`nuance`・発音記号`ipa`。2026-08-11にsyn/nuance、2026-08-15に全2,029語へipa付与済み。ipaは米音IPAでスラッシュ無し、句は語ごとにスペース区切り）②`VISUAL_OVERRIDES`（フラッシュカードのイラスト修正版。`{e:絵文字, h:情景ヒント}`）。キーはどちらもglossaryの`word.toLowerCase()`。**新しいエッセイを追加したら、新出単語分を両方に追加する（synは2〜4個の同義語、nuanceは語感+使用場面を日本語30〜70字）**
...
## エッセイ規模
- エッセイは242本（2026-08-16時点。data.jsのESSAYS配列。うちReal Conversations 42本、Vocab Review Conversations 42本）。2026-08-16に全8ジャンル×2本のショート版（85〜100語）を追加、glossary語彙は2,088語に増加（新出59語はenrichment・VISUAL_OVERRIDES・ipa完備）
...
- **穴埋めクイズ**（Quizタブの「✏️ 穴埋め」。2026-08-11に通訳練習モードを廃止して置き換え、2026-08-14に入力式→カード式へ変更）: Glossaryの例文の対象語を空所にする。**空所には頭文字＋残り字数分の全角下線**（`clozeBlankText()`が `I＿＿＿＿` を生成）、**日本語訳は対応部分に下線**（`clozeJaSpan()`が訳語と例文訳の最長共通部分文字列を探す）。別途のヒント行は出さない。**入力欄は廃止し、カードをタップして答えを見る単語カード形式**。1セッション10問、`dailyStats[date].cloze`に記録
- **穴埋めの出題条件（重要）**: `buildClozeQueue()`は「①例文に対象語が出現する（`clozeWordPattern()`。活用・子音重複・y→ie・e脱落・`CLOZE_IRREGULAR`の不規則変化を許容）」かつ「②訳文に対応部分が2文字以上見つかる」語だけを出題する。両方を満たすので**空所と日本語下線が必ず揃う**。2026-08-14時点で1,791語が対象
...
- **ゲーミフィケーション（2026-08-16追加、同日に継続軸を撤廃）**: ホーム上部に4要素。①**ネイティブ度メーター**: 語彙35%（マスター語数/2029）+読書量35%（累計語数/100万）+演習30%（クイズ+穴埋め+英作文+チェックの累計/2万回）の加重スコア。**経過時間に依存する軸は置かない方針（ユーザー指示: レベルアップは本人の努力量だけで決まるようにする）。継続日数はメーターに入れず、ホームのStudy Daysタイル・ストリークバナー・SRSタブの学習日数・連続学習バッジで記録し続ける**。ランクは B2(<25) / C1(25+) / C2(50+) / Near-Native(75+) / NATIVE(100)。目標値は`NATIVE_GOALS`（ユーザー選択の「スタンダード」設定。変更時は本人確認）②**XPレベル**: `dayXP()`が日次XPを計算（読了0.1/語・エッセイ5・クイズ2・チェック1・英作文/穴埋め3＋ミッション達成各+20）。レベル曲線は`xpForLevel(n)=80(n-1)^1.6`③**デイリーミッション**: `missionsForDate()`が日付から決定的に3つ選ぶ（保存不要・過去分も再計算可能）④**実績バッジ**: 32種を現在の統計から判定。**設計原則: 全て既存記録（dailyStats/vocabulary/readCounts）からの導出で、新規の永続化・Sync変更は無し**。SRSタブに獲得XPグラフとサマリー行も追加
- **単語カードの絵文字（2026-08-16追加）**: Vocabタブの各カードの**見出し語の右**（品詞の後ろ）に、クイズのフラッシュカードと同じ絵文字を表示（`.vocab-emoji`、`getWordVisual()`から取得）。同じ語なら両画面で必ず同じ絵柄になる
